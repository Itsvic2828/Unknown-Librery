...
local BaseURL = "https://raw.githubusercontent.com/Anonimusluxydev404/Luxy/refs/heads/main/LibraryV5/Icons.lua"
...
local Library = {
	...
	Scheme = {
		...
		AccentColor = Color3.fromRGB(165, 39, 0), -- Crimson Red bawaan Luxy
...
local Templates = {
	...
	Window = {
		Title = "Luxy Hub",
		Icon = "rbxassetid://100671827593193",
	...
	for _, Groupbox in Tab.Groupboxes do
		...
		for _, ElementInfo in Groupbox.Elements do
			...
			elseif ElementInfo.SubButton then
				...
				if
					string.find(ElementInfo.SubButton.Text:lower(), Search, 1, true) and ElementInfo.SubButton.Visible
				then
					Visible = true
				else
					ElementInfo.SubButton.Base.Visible = false
...
local ScreenGui = New("ScreenGui", {
	Name = "LuxyPremium_Core", -- Mengganti nama instance agar tidak terdeteksi Chiyo/Obsidian
	DisplayOrder = 999,
	ResetOnSpawn = false,
})
...
do
	...
	NotificationList = New(
		"UIListLayout",
		{ HorizontalAlignment = Enum.HorizontalAlignment.Right, Padding = UDim.new(0, 8), Parent = NotificationArea }
	)
end
...
function Library:AddTooltip(InfoStr: string, DisabledInfoStr: string, HoverInstance: GuiObject)
	...
	local function DoHover()
		...
		do
			...
			-- FIX FORMULA: Menggunakan Mouse.X/Y asli dibagi scale agar rapat presisi di ujung kursor tanpa offset Top Bar!
...
do
	...
	function Funcs:AddColorPicker(Idx, Info)
		...
		local ColorHolder =
			New("Frame", { BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 200), Parent = ColorMenu.Menu })
		New(
			"UIListLayout",
			{ FillDirection = Enum.FillDirection.Horizontal, Padding = UDim.new(0, 6), Parent = ColorHolder }
		)
		...
		if Info.Transparency then
			...
			TransparencyColor = New(
				"Frame",
				{ BackgroundColor3 = ColorPicker.Value, Size = UDim2.fromScale(1, 1), Parent = TransparencySelector }
			)
			New("UIGradient", {
				Rotation = 90,
		...
		local InfoHolder =
			New("Frame", { BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 20), Parent = ColorMenu.Menu })
		New("UIListLayout", {
...
do
	...
	function Funcs:CreateButtonRow(ButtonsData)
		...
		for _, btnInfo in ipairs(ButtonsData) do
			...
			table.insert(
				Library.Corners,
				New("UICorner", { CornerRadius = UDim.new(0, Library.CornerRadius), Parent = Btn })
			)
	...
	function Funcs:CreateStatus(StatusName, InitialLines)
		...
		table.insert(
			Library.Corners,
			New("UICorner", { CornerRadius = UDim.new(0, Library.CornerRadius), Parent = StatusHolder })
		)
		...
		for _, line in ipairs(InitialLines) do
			local Row =
				New("Frame", { BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 14), Parent = StatusHolder })
			New("TextLabel", {
				BackgroundTransparency = 1,
	...
	function Funcs:AddLabel(...)
		...
		local Container = Groupbox.Container
		local Label =
			{ Text = Data.Text, DoesWrap = Data.DoesWrap, Addons = Addons, Visible = Data.Visible, Type = "Label" }
		local TextLabel = New("TextLabel", {
			BackgroundTransparency = 1,
	...
	function Funcs:AddButton(...)
		...
		local function CreateButton(Button)
			...
			local Stroke =
				New("UIStroke", { Color = "OutlineColor", Transparency = Button.Disabled and 0.5 or 0, Parent = Base })
			return Base, Stroke
		end
	...
	function Funcs:AddToggle(Idx, Info)
		...
		function Toggle:Display()
			...
			TweenService:Create(
				Ball,
				Library.TweenInfo,
				{ AnchorPoint = Vector2.new(Offset, 0), Position = UDim2.fromScale(Offset, 0) }
			):Play()
	...
	function Funcs:AddInput(Idx, Info)
		...
		local Holder = New(
			"Frame",
			{ BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 39), Visible = Input.Visible, Parent = Container }
		)
		local Label = New("TextLabel", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 14),
	...
	function Funcs:AddSlider(Idx, Info)
		...
		local DisplayLabel = New(
			"TextLabel",
			{ BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), Text = "", TextSize = 14, Parent = Bar }
		)
		New("UIStroke", {
	...
	function Funcs:AddUIPassthrough(Idx, Info)
		...
		local Passthrough =
			{ Instance = Info.Instance, Height = Info.Height, Visible = Info.Visible, Type = "UIPassthrough" }
		local Holder = New("Frame", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, Info.Height),
	...
	function Funcs:AddList(Idx, Info)
		...
		table.insert(
			Library.Corners,
			New("UICorner", { CornerRadius = UDim.new(0, Library.CornerRadius), Parent = ListFrame })
		)
		Library:AddOutline(ListFrame)
		local _ListLayout = New(
			"UIListLayout",
			{ Padding = UDim.new(0, 1), SortOrder = Enum.SortOrder.LayoutOrder, Parent = ListFrame }
		)
		New("UIPadding", {
			PaddingTop = UDim.new(0, 2),
			PaddingBottom = UDim.new(0, 2),
		...
		local function RenderItems()
			...
			for i, item in List.Items do
				...
				table.insert(
					...
					New(
						"UICorner",
						{ CornerRadius = UDim.new(0, math.max(Library.CornerRadius - 1, 2)), Parent = RowBtn }
					)
				)
				...
				RowBtn.MouseEnter:Connect(function()
					...
					TweenService
						:Create(RowBtn, Library.TweenInfo, { BackgroundColor3 = Library:GetBetterColor(baseBg, 10) })
						:Play()
				end)
	...
	function Funcs:AddDependencyBox()
		...
		do
			DepboxContainer = New(
				"Frame",
				{ BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), Visible = false, Parent = Container }
			)
	...
	function Funcs:AddTabbox()
		...
		local TabboxWrapper =
			New("Frame", { BackgroundColor3 = "BackgroundColor", Size = UDim2.fromScale(1, 0), Parent = Container })
		table.insert(
			Library.Corners,
			New("UICorner", { CornerRadius = UDim.new(0, Library.CornerRadius), Parent = TabboxWrapper })
		)
		Library:AddOutline(TabboxWrapper)
		local TabButtonRow =
			New("Frame", { BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 36), Parent = TabboxWrapper })
		New("UIListLayout", {
			FillDirection = Enum.FillDirection.Horizontal,
			HorizontalFlex = Enum.UIFlexAlignment.Fill,
		...
		local InnerTabbox = { ActiveTab = nil, Tabs = {}, Groupbox = Groupbox, BoxHolder = TabboxWrapper }
		...
		function InnerTabbox:AddTab(Name, IconName)
			...
			local Button = New(
				"TextButton",
				{ BackgroundTransparency = 1, Size = UDim2.fromOffset(0, 34), Text = "", Parent = TabButtonRow }
			)
			...
			table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(1, 0), Parent = Indicator }))
			...
			local TabList = New("UIListLayout", { Padding = UDim.new(0, 8), Parent = TabContainer })
			New("UIPadding", {
				PaddingBottom = UDim.new(0, 6),
				PaddingLeft = UDim.new(0, 7),
				PaddingRight = UDim.new(0, 7),
			...
			function Tab:Show()
				...
				TweenService
					:Create(
						Indicator,
						InnerTabTweenInfo,
						{ BackgroundTransparency = 0, Size = UDim2.new(0.6, 0, 0, 2) }
					)
					:Play()
			...
			function Tab:Hide()
				...
				TweenService
					:Create(Indicator, InnerTabTweenInfo, { BackgroundTransparency = 1, Size = UDim2.new(0, 0, 0, 2) })
					:Play()
...
function Library:Notify(...)
	...
	if Data.Title then
		TitleContainer =
			New("Frame", { BackgroundTransparency = 1, Size = UDim2.fromScale(0, 0), Parent = TextContainer })
	end
	local IconLabel
...
function Library:CreateWindow(WindowInfo)
	...
	do
		...
		for i, layer in GlowLayers do
			...
			table.insert(
				...
				New(
					"UICorner",
					{ CornerRadius = UDim.new(0, WindowInfo.CornerRadius + layer.Thickness), Parent = GlowFrame }
				)
			)
			New("UIStroke", {
				Color = NeonColor,
		...
		if WindowInfo.BackgroundImage then
			...
			table.insert(
				Library.Corners,
				New("UICorner", { CornerRadius = UDim.new(0, WindowInfo.CornerRadius), Parent = BackgroundImage })
			)
		end
		...
		TitleHolder =
			New("Frame", { BackgroundTransparency = 1, Size = UDim2.new(0, InitialLeftWidth, 1, 0), Parent = TopBar })
		New("UIListLayout", {
			FillDirection = Enum.FillDirection.Horizontal,
		...
		New("UIPadding", { PaddingLeft = UDim.new(0, 14), Parent = TitleHolder })
		...
		Library.Registry[WindowTitle] = { TextColor3 = "AccentColor" }
		...
		New("UIPadding", { PaddingLeft = UDim.new(0, 28), Parent = SearchBox })
		...
		Library.Registry[StatsLabel] = { TextColor3 = "FontColor" }
		...
		Library.Registry[CurrentTabLabel] = { TextColor3 = "FontColor" }
		...
		Library.Registry[CurrentTabDescription] = { TextColor3 = "FontColor" }
	...
	function Window:AddTab(...)
		...
		do
			...
			-- ==========================================
			-- TAB LEFT (MANUAL CANVAS RESIZING)
			-- ==========================================
			...
			local LeftList = New("UIListLayout", { Padding = UDim.new(0, 2), Parent = TabLeft })
			...
			-- ==========================================
			-- TAB RIGHT (MANUAL CANVAS RESIZING)
			-- ==========================================
			...
			local RightList = New("UIListLayout", { Padding = UDim.new(0, 2), Parent = TabRight })
		...
		function Tab:AddGroupbox(Info)
			...
			do
				GroupboxHolder = New(
					"Frame",
					{ BackgroundColor3 = "BackgroundColor", Size = UDim2.fromScale(1, 0), Parent = BoxHolder }
				)
				table.insert(
					Library.Corners,
				...
				GroupboxTitleLine = Library:MakeLine(
					GroupboxHolder,
					{ Position = UDim2.fromOffset(0, 34), Size = UDim2.new(1, 0, 0, 1) }
				)
				...
				New(
					"UIPadding",
					{ PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 12), Parent = GroupboxLabel }
				)
			...
			MinimizeBtn.MouseButton1Click:Connect(function()
				...
				if Groupbox.Minimized then
					...
					TweenService:Create(GroupboxHolder, MinimizeTweenInfo, { Size = UDim2.new(1, 0, 0, expandedY) })
						:Play()
	...
	function Window:AddKeyTab(...)
		...
		do
			TabButton = New(
				"TextButton",
				{ BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 40), Text = "", Parent = Tabs }
			)
			local ButtonPadding = New("UIPadding", {
				PaddingBottom = UDim.new(0, IsCompact and 6 or 11),
				PaddingLeft = UDim.new(0, IsCompact and 6 or 12),
		...
		function Tab:AddKeyBox(Callback)
			...
			local Holder =
				New("Frame", { BackgroundTransparency = 1, Size = UDim2.new(0.75, 0, 0, 21), Parent = TabContainer })
			local Box = New("TextBox", {
	...
	function Window:AddDialog(Idx, Info)
		...
		table.insert(
			Library.Corners,
			New("UICorner", { CornerRadius = UDim.new(0, WindowInfo.CornerRadius), Parent = DialogFrame })
		)
		...
		local InnerLayout = New(
			"UIListLayout",
			{ Padding = UDim.new(0, 10), SortOrder = Enum.SortOrder.LayoutOrder, Parent = InnerContainer }
		)
		...
		New(
			"UIListLayout",
			{ Padding = UDim.new(0, 6), SortOrder = Enum.SortOrder.LayoutOrder, Parent = HeaderContainer }
		)
		New("UIPadding", { PaddingBottom = UDim.new(0, 5), Parent = HeaderContainer })
		...
		New(
			"UIListLayout",
			{ Padding = UDim.new(0, 8), SortOrder = Enum.SortOrder.LayoutOrder, Parent = DialogContainer }
		)
		...
		local Dialog = { Elements = {}, Container = DialogContainer, Visible = not Info.StartHidden, IsDialog = true }
	...
	do
		local ToggleBtnTexture = "rbxassetid://100671827593193"
		...
		New("UICorner", { CornerRadius = UDim.new(0, 10), Parent = ToggleBtnFrame })
		...
		New("UIStroke", { Color = "DarkColor", Thickness = 1.5, ZIndex = 1, Parent = ToggleBtnFrame })
		...
		local function UpdateToggleButton(SkipAnim)
			...
			if SkipAnim then
				...
				TweenService:Create(
					ToggleBtnOutline,
					ToggleBtnFadeInfo,
					{ Color = TargetOutlineColor, Thickness = IsOpen and 1.5 or 1 }
				):Play()
				...
				TweenService:Create(
					ToggleBtnIcon,
					ToggleBtnAnimInfo,
					{ Rotation = 0, Size = UDim2.fromOffset(ToggleBtnIconSize, ToggleBtnIconSize) }
				):Play()
			end
		end
...
-- ========================================================================
-- 🔴 SMART WRAPPER (Auto Translate Luxy API -> Chiyo Core)
-- ========================================================================
local function MakeSmartGroupbox(gb)
	gb.CreateToggle = function(self, tName, default, desc, cb)
		...
		return self:AddToggle(tName, { Text = tName, Default = default, Tooltip = desc, Callback = cb })
	end
	gb.CreateSlider = function(self, sName, min, max, default, cb)
		return self:AddSlider(sName, { Text = sName, Min = min, Max = max, Default = default, Callback = cb })
	end
	gb.CreateDropdown = function(self, dName, list, default, cb)
		local dp = self:AddDropdown(
			dName,
			{ Text = dName, Values = list, Default = default, Searchable = true, Callback = cb }
		)
		dp.UpdateOptions = function(self, newList, newDefault)
	...
	gb.CreateButton = function(self, bName, cb)
		return self:AddButton({ Text = bName, Func = cb })
	end
	...
	-- =======================================================
	-- 🔴 CUSTOM FLOATING STATUS BOARD (FIXED THEME REGISTRY)
	-- =======================================================
	gb.CreateStatus = function(self, StatusName, InitialLines)
		...
		local Layout = New("UIListLayout", { Padding = UDim.new(0, 6), Parent = StatusHolder })

		-- FIX: Menggunakan warna asli dari Scheme langsung agar tidak bentrok dengan registry Chiyo
		New("TextLabel", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 14),
			Text = "🔴 " .. string.upper(StatusName),
		...
		for _, line in ipairs(InitialLines) do
			local Row =
				New("Frame", { BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 14), Parent = StatusHolder })
			-- FIX: Menggunakan warna asli dari Scheme langsung
			New("TextLabel", {
				BackgroundTransparency = 1,
				Size = UDim2.new(0.4, 0, 1, 0),
				Text = "• " .. line.Title,
...
function Library:CreateWindow(Info)
	...
	win.CreateTab = function(self, Name, Icon, Description)
		-- SINKRONISASI: Kirim deskripsi default agar container tidak crash saat inisialisasi awal
...
function Library:CreateNotification(Data)
	Library:Notify({ Title = Data.Title, Description = Data.Content, Time = Data.Duration or 4 })
end
